...
| World Bank Global Administrative Divisions | Jun/Sep 2025 | CC-BY 4.0 | `data/WB_shapes/raw/`, derived `web/data/wb_*.json` |
...
- World Bank Global Administrative Divisions (GAD) boundaries — CC-BY 4.0 — https://datacatalog.worldbank.org/search/dataset/0038272
  (raw WB Official Boundaries shapefile zips vendored in `data/WB_shapes/raw/`; topologically
  simplified by `simplify_shapes.sh` (mapshaper) and converted to the JSON assets in `web/data/`
  by `prepare_shapes.py`)
